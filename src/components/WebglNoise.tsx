...
// Inline the noise shader so it doesn't depend on the DOM
const noiseShader = `
vec3 mod289(vec3 x) {
    ...
    class Molecule extends THREE.Object3D {
      ...
      build() {
        ...
        this.material = new THREE.PointsMaterial({
          ...
          color: 0x2d5a3d, // Subtle, darker forest green
          depthTest: false
        });
      ...
      dot(size = 32, color = "#2d5a3d") {
        ...
        ctx.fillStyle = color; // Green instead of white
        ctx.fill(circle);
      ...
      animate(time: number) {
        ...
        if ((this.material.userData as any).shader)
        (this.material.userData as any).shader.uniforms.time.value = time;
      }
    }
